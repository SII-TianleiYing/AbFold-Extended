...
def train(dataloader1, dataloader2, student, teacher, loss_fn, optimizer, scheduler, epoch):
    ...
    for i, (X2, y2) in enumerate(tqdm(dataloader2)):
        # X2, y2 = L2
        # X = cat_input(X1, X2)
        # X, y2 = tensor_dict_to_device(X, device), tensor_dict_to_device(y2, device)

        X, y2 = tensor_dict_to_device(X2, device), tensor_dict_to_device(y2, device)
        ...
        # loss = loss_fn(pred_student2, y2)
        # loss, losses = loss_fn(pred_student2, y2, _return_breakdown=True)
        loss, losses = loss_fn(pred_student, y2, _return_breakdown=True)
        ...
        writer.add_scalar('learning_rate', curr_lr, epoch * size + i)
        writer.add_scalar('loss', loss, epoch * size + i)
        writer.add_scalar('loss/fape', losses['fape'], epoch * size + i)
        writer.add_scalar('loss/supervised_chi', losses['supervised_chi'], epoch * size + i)
        writer.add_scalar('loss/violation', losses['violation'], epoch * size + i)
        writer.add_scalar('loss/bondlen', losses['bondlen'], epoch * size + i)
        writer.add_scalars('losses', {
            'cum_loss': loss,
            'fape': losses['fape'],
            'supervised_chi': losses['supervised_chi'],
            'violation': losses['violation'],
            'bondlen': losses['bondlen']
        }, epoch * size + i)
        writer.add_scalar('iter_time(s)', time_iter_end-time_iter_start, epoch * size + i)
...
def main(args):
    ...
    non_struct_data = UnsupervisedDataset(unsupervised_repr_dir, unsupervised_fasta_dir, pdb_dir=args.non_struct_pdb_dir,
                                          point_feat_dir=args.non_struct_point_feat_dir)
    non_struct_dataloader = DataLoader(non_struct_data, batch_size=non_struct_batch_size, num_workers=args.num_workers,
                                       shuffle=True, pin_memory=True, persistent_workers=True, drop_last=True)
    struct_batch_size = batch_size - non_struct_batch_size
    struct_data = AbFoldDataset(label_dir, pdb_dir=args.pdb_dir, point_feat_dir=args.point_feat_dir)
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        optimizer.step()
        print(f"Epoch {epoch}\n-------------------------------")
        config_change_epoch1, config_change_epoch2, config_change_epoch3 = 80, 160, 240
        if epoch <= config_change_epoch1:
            config.loss.fape.weight = 10.0
            config.loss.bondlen.weight = 1.0
            config.loss.violation.weight = 0.1
            config.loss.supervised_chi.weight = 0.0
        # elif config_change_epoch1 <= epoch < config_change_epoch2:
        #     config.loss.fape.weight = 1.0
        #     config.loss.bondlen.weight = 1.0
        #     config.loss.violation.weight = 0.0
        #     config.loss.supervised_chi.weight = 0.0
        # elif config_change_epoch2 <= epoch < config_change_epoch3:
        #     config.loss.fape.weight = 1.0
        #     config.loss.bondlen.weight = 1.0
        #     config.loss.violation.weight = 1.0
        #     config.loss.supervised_chi.weight = 0.0
        else:
            config.loss.fape.weight = 10.0
            config.loss.bondlen.weight = 1.0
            config.loss.violation.weight = 1.0
            config.loss.supervised_chi.weight = 1.0
        ...
        if epoch in [config_change_epoch1, config_change_epoch2, config_change_epoch3]:
            torch.save(checkpoint, f'{checkpoint_name}_{epoch}')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--unsupervised_repr_dir", type=str,
                        default="/home/pengchao/data/abfold/train/unsupervised_repr",
                        help="Directory containing precomputed representations")
    parser.add_argument("--unsupervised_fasta_dir", type=str,
                        default="/home/pengchao/data/abfold/train/unsupervised_fasta",
                        help="Directory containing fasta files")
    parser.add_argument("--label_dir", type=str, default="/home/pengchao/data/abfold/train/supervised_label",
    ...
    parser.add_argument("--gpus", type=str, default='1,2', help="use comma as separator")
    parser.add_argument("--max_lr", type=float, default=0.0001, help="max learning rate")
    parser.add_argument("--batch_size", type=int, default=8, help="training gpu")
    ...
    parser.add_argument("--non_struct_loss_ratio", type=float, default=0.1,
                        help="the ratio of non-structure data per training epoch")
    parser.add_argument('--num_workers', default=1, type=int, help='Number of data loading workers per GPU.')
    parser.add_argument("--pdb_dir", type=str, default="/home/pengchao/data/abfold/train/point_feat/igfold_predict",
                        help="Directory containing pdb files predicted by igfold")
    parser.add_argument("--non_struct_pdb_dir", type=str,
                        default="/home/pengchao/data/abfold/train/point_feat/unlabeled_igfold_predict",
                        help="Directory containing pdb files predicted by igfold")
    parser.add_argument("--point_feat_dir", type=str, default="/home/pengchao/data/abfold/train/igfold_embedding/supervised",
                        help="Directory containing embedding files generated by point_mae")
    parser.add_argument("--non_struct_point_feat_dir", type=str,
                        default="/home/pengchao/data/abfold/train/igfold_embedding/unsupervised",
                        help="Directory containing embedding files generated by point_mae")
