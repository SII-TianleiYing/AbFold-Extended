...
seq_to_cdr_mask = {}
def get_CDRs_mask_with_anarci(sequence, chain_type, only_H3=False):
    """
    使用ANARCI工具从抗体序列中获取CDR区
    """
    ...
    # Extract CDRs based on the IMGT numbering
    ...
    for i, ((pos, _), aa) in enumerate(numbered_seq):
        if chain_type == 'L':  # Light chain
            if 24 <= pos <= 34:
                CDRs["CDR1"].append(i)
            elif 50 <= pos <= 56:
                CDRs["CDR2"].append(i)
    ...
    cdr_mask = torch.zeros([len(sequence)], dtype=torch.int32, requires_grad=False)
    trunk_mask = torch.ones([len(sequence)], dtype=torch.int32, requires_grad=False)
    noise_ratio = torch.zeros([len(sequence)], dtype=torch.float32, requires_grad=False)
    if only_H3:
        if chain_type == 'H':
            for pos in CDRs["CDR3"]:
                cdr_mask[pos] = 1
                trunk_mask[pos] = 0

        return cdr_mask, trunk_mask, noise_ratio
    ...
    decay_rate = 0.5
    cdr_pos = CDRs["CDR1"] + CDRs["CDR2"] + CDRs["CDR3"]
    cdr_pos_np = np.array(cdr_pos)
    for i in range(len(sequence)):
        distances = np.abs(i - cdr_pos_np)
        dist = np.min(distances)

        noise_ratio[i] = 0.01 + 0.99 * math.exp(-decay_rate * dist)

    seq_to_cdr_mask[sequence] = (cdr_mask, trunk_mask, noise_ratio)
    return cdr_mask, trunk_mask, noise_ratio
...
def get_features_from_seq(seq):
    ...
    array of shape (L, 20), where L is the sequence length.
    """
    for aa_i in seq:
    ...
    features = torch.zeros((n_res, n_features))
    # Residues one hot encoding.
    for i, aa_i in enumerate(seq):
        features[i, aa_list.index(aa_i)] = 1
    return features
