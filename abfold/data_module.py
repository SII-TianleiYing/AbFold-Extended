...
class AbFoldDataset(Dataset):
    def __init__(self, label_dir, pdb_dir, point_feat_dir):
    ...
    def __getitem__(self, idx):
        ...
        pdb_path = os.path.join(self.pdb_dir, f'{self._label_ids[idx].split(".")[0]}.pdb')

        point_feat_path = os.path.join(self.point_feat_dir, f'{self._label_ids[idx].split(".")[0]}.pt')
        igfold_emb = torch.load(point_feat_path, map_location=torch.device('cpu'))
        ...
        if label_h['aatype'].shape[0] + label_l['aatype'].shape[0] != point_feat.shape[0]:
            ...
            plddt = plddt[:label_h['aatype'].shape[0] + label_l['aatype'].shape[0]]
        ...
        plddt_h, plddt_l = torch.split(plddt, [label_h['aatype'].shape[0], label_l['aatype'].shape[0]])
        label_h['plddt'] = plddt_h
        label_l['plddt'] = plddt_l
        ...
        plddt_h, plddt_l = label_h.pop('plddt'), label_l.pop('plddt')
        ...
        plddt = torch.cat([plddt_h, plddt_l])

        # split two chains by adding 1024 in L chain index
        res_idx = torch.cat([label_h['residue_index'], label_l['residue_index'] + 256], dim=-1)

        feature = {'s': s, 'z': z, 'aatype': aatype, 'point_feat': point_feat, 'plddt': plddt, 'res_idx': res_idx, 'repr_id': self._label_ids[idx].split(".")[0]}
...
class UnsupervisedDataset(Dataset):
    def __init__(self, repr_dir, fasta_dir, pdb_dir, point_feat_dir):
    ...
    def __getitem__(self, idx):
        ...
        point_feat_path = os.path.join(self.point_feat_dir, f'{repr_id}.pt')
        ...
        if len(seq_h) + len(seq_l) != point_feat.shape[0]:
            ...
            plddt = plddt[:len(seq_h) + len(seq_l)]
        point_feat_h, point_feat_l = torch.split(point_feat, [len(seq_h), len(seq_l)])
        plddt_h, plddt_l = torch.split(plddt, [len(seq_h), len(seq_l)])
        ...
        # split two chains by adding 1024 in L chain index
        res_idx = torch.cat([torch.arange(0, aatype_h.shape[-1]), torch.arange(0, aatype_l.shape[-1]) + 1024], dim=-1)

        feature = {'s': s, 'z': z, 'aatype': aatype, 'point_feat': point_feat, 'plddt': plddt, 'res_idx': res_idx}
